...
public class BbsDao {
	...
	public int updateCon(BbsVo v) {
		int re = -1;
		String sql = "update bbs set title=?, content=?, fname=? where pwd=? and num=?";
		try {
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, v.getTitle());
			pstmt.setString(2, v.getContent());
			pstmt.setString(3, v.getFname());
			pstmt.setString(4, v.getPwd());		
			pstmt.setInt(5, v.getNum());
...
public ArrayList<BbsVo> getBbs2(int num){
