...
import com.bit.action.BbsDetailAction;
import com.bit.action.BitAction;
import com.bit.action.JoinAction;
import com.bit.action.JoinOkAction;
import com.bit.action.ListBbsAction;
...
public class BitController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			while((line = br.readLine())!=null)
			{
				...
				map.put(cmd, (BitAction)Class.forName(clsName).newInstance());//클래스명의 문자열을 가지고 객체를 생성
				//map.put(cmd, obj);//클래스명의 문자열을 가지고 객체를 생성
	...
	private void proRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String cmd = uri.substring(uri.lastIndexOf("/")+1);//요청한 uri의 서비스명만 추출\
